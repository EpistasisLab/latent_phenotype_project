...
if env_name == ['874-average', '894-average', '914-average']:
    # 54882 nans for 874, 97816 nans for 894, 173874 nans for 914: assuming nan is 0
    env_data = pd.read_csv(path, delimiter = "\t", usecols = (["eid"] + env_name))
    exercise = env_data[env_name].to_numpy()
    exercise[np.isnan(exercise)] = 0
    # assuming 16 hours of exercise in a day as the maximum (960 minutes)
    # replaces only 497 values with the median
    bad_indices = np.sum(exercise, axis = 1) > 960
    exercise_score = np.sum(np.array([1, 2, 3])*exercise, axis = 1)
    exercise_score[bad_indices] = np.median(exercise_score[bad_indices == False])
...
phenotypes = phenotypes_info.loc[:, np.arange(17).astype(str)].to_numpy()
for i in range(17): 
...
phenotypes = (phenotypes - np.nanmean(phenotypes))/np.nanstd(phenotypes)
