...
if not os.path.exists("vQTL_output"):
    try: 
        os.mkdir("vQTL_output")
    except:
        pass
...
for i in range(16): 
    ...
    phenotypes[val_indices , i] = np.abs(phenotypes[val_indices , i] - np.median(phenotypes[val_indices , i]))
if chr == '1' and pheno == 0:
    pd.DataFrame(phenotypes).to_csv("vQTL_phenotypes.txt", sep = "\t", header = False, index = False)
...
for o, label in zip(out, ["_male", "_female"]):
    ...
    o_df["rsID"] = rsIDs
    o_df.to_csv("vQTL_output/vQTL_effects_chr" + chr + "_P" + str(pheno) + label + ".txt", sep = "\t", header = True, index = False)