...
for i in range(16): 
    phenotypes[:, i] = yj(phenotypes[:, i])[0]
    phenotypes[:, i] = np.abs(phenotypes[:, i] - np.median(phenotypes[:, i]))
...
out.to_csv("vQTL_output/vQTL_effects_chr" + chr + "_P" + str(pheno) + ".txt", sep = "\t", header = True, index = False)