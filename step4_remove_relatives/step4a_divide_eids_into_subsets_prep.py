...
eids_fam = pd.read_csv("UKB_samples_filtered_with_remainder.fam", delim_whitespace = True, usecols = [0], header = None)
eids_fam.columns = ["eid"]
unfiltered_eids = pd.read_csv("../step1_get_phenotypes_simple/y.txt", delimiter = "\t", usecols = ["eid", "any_HF"], header = 0)
eids_fam_with_status = eids_fam.merge(unfiltered_eids, on = "eid", how = "inner")
remainder = len(eids_fam)%10
eids_fam_no_HF = eids_fam_with_status.loc[eids_fam_with_status["any_HF"] == False, ["eid"]]
remainder_eids = eids_fam_no_HF.loc[0:(remainder - 1), ["eid"]]
remainder_eids[["eid", "eid"]].to_csv("remainder_eids.tab", sep = "\t", header = False, index = False)
eids_fam = eids_fam.loc[eids_fam["eid"].isin(remainder_eids.to_numpy().reshape(-1)) == False , :]
eids_fam = eids_fam.loc[:, ["eid", "eid"]]
