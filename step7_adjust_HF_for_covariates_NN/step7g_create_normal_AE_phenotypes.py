...
name = "CV_normalAE_" + str(dop) + "dop_" + str(rs) + "rs_"  + str(ws) + "ws_" + str(dim1) + "d1_" + str(dim2) + "d2.txt"
model_path = "normalAE_CV_fold_networks/" + name[:-4] + ".model"
folds = [1, 2, 3, 4, 5]
for train_inds, test_inds, CV in zip(training_inds, testing_inds, folds):
    ...
    model_path = "normalAE_CV_fold_networks/fold" + str(CV) + "_" + name[:-4] + ".model"
    ...
    fold_errs.to_csv("normalAE_fold_error_dfs/fold" + str(CV) + "_" + name, sep = "\t", header = True, index = False)
