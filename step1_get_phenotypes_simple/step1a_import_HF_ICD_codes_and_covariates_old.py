...
other_ICD_cols = np.setdiff1d(all_ICD_cols, main_ICD_cols)

all_ICD_date_cols = ["date_" + code for code in ICD_codes]
main_ICD_date_cols = ["date_" + code for code in main_ICD_codes]
other_ICD_date_cols = np.setdiff1d(all_ICD_date_cols, main_ICD_date_cols)

all_ICD_data = pd.DataFrame(combined_set)
all_ICD_data.columns = all_ICD_cols
main_ICD_data = all_ICD_data[main_ICD_cols]
other_ICD_data = all_ICD_data[other_ICD_cols].to_numpy()
del all_ICD_data
del combined_set
...
pca.fit(other_ICD_data.astype(float))
end = time.time()
print("pca1 finished after " + str(end - start) + " seconds")
other_ICD_data_PCs = pca.transform(other_ICD_data.astype(float))
main_ICD_data.to_csv("ICD_data_main.txt", sep = "\t", header = True, index = False)
other_ICD_data_PCs = pd.DataFrame(other_ICD_data_PCs)
other_ICD_data_PCs.to_csv("ICD_data_other_PCs.txt", sep = "\t", header = False, index = False)

ICD_eids.to_csv("ICD_eids.txt", sep = "\t", header = True, index = False)
del other_ICD_data_PCs
del other_ICD_data

all_ICD_dates.columns = all_ICD_date_cols
main_ICD_dates = all_ICD_dates[main_ICD_date_cols].to_numpy(dtype = float)
main_ICD_dates[main_ICD_dates == 0] = np.inf
main_inv_ICD_dates = pd.DataFrame(1/main_ICD_dates)
main_inv_ICD_dates.columns = main_ICD_date_cols
main_inv_ICD_dates.to_csv("ICD_data_dates_main.txt", sep = "\t", header = True, index = False)
other_ICD_dates = all_ICD_dates[other_ICD_date_cols].to_numpy(dtype = float)
other_ICD_dates[other_ICD_dates == 0] = np.inf
other_inv_ICD_dates = 1/other_ICD_dates
del all_ICD_dates

print("pca2 starting")
start = time.time()
pca2 = PCA(n_components = 1000)
pca2.fit(other_inv_ICD_dates.astype(float))
end = time.time()
print("pca2 finished after " + str(end - start) + " seconds")
other_inv_ICD_dates_PCs = pca2.transform(other_inv_ICD_dates.astype(float))
other_inv_ICD_dates_PCs = pd.DataFrame(other_inv_ICD_dates_PCs)
other_inv_ICD_dates_PCs.to_csv("ICD_data_dates_other_PCs.txt", sep = "\t", header = False, index = False)
del other_inv_ICD_dates
