...
#---------------------------------------------------------------------------------------------------
#
# This part's only purpose is to correlate my EDGE2 p values with the permutation p values
# 
#---------------------------------------------------------------------------------------------------

QTL_suffixes = ["alcohol", "exercise", "smoking", "gender"]
paths = ["hits_QTL_" + i for i in QTL_suffixes]
files = []
for path in paths:
...
plt.plot(-np.log10(pvals["p"]), -np.log10(pvals["pEDGE2"]), "ok", label = "r = " + str(np.round(corr, 2)))
plt.legend(fontsize=20)
plt.xticks(fontsize=20)
plt.yticks(fontsize=20)
plt.xlabel("-log10(p_nominal/p_main)", fontsize=20)
plt.ylabel("-log10(p_permutation)", fontsize=20)
...
main_effect_counts = [np.sum(main_effects["pheno_index"] == i) for i in range(16)]
GxE_effect_counts = []
...
for eff_name in column_names[2:]:
    ...
    GxE_effect_counts.append([np.sum(GxE_eff_name["pheno_index"] == i) for i in range(16)])
step10e_count_data = [[i + 1 for i in range(16)]] + [main_effect_counts] + GxE_effect_counts
...
names = ["_alcohol", "_smoking", "_gender", "_exercise"]
            ...
            for j in num_E:
                ...
                if len(geno_sub) > 0:
                    ...
                    for g1, g2, g2_null, rsID in zip(geno_sub.T, geno_EDGE2.T,  geno_EDGE2_null.T, rsIDs_j):
                        ...
                        standard_p_values_pheno.append(i)
                        
                        figureS4_rsIDs = ['rs67183339', 'rs6698949']

                        if rsID in figureS4_rsIDs:
                            if rsID == 'rs67183339':
                                letter = "b"
                            else:
                                letter = "a"
                            env_factor_name = all_E_names[j]
                            E_low, E_high = E < np.mean(E), E > np.mean(E)
                            g_low, g_high = g1[E_low], g1[E_high]
                            p_low, p_high = p[E_low], p[E_high]
                            X_domain = [0, 1, 2]
                            Y_dists_low = [p_low[g_low == 0], p_low[g_low == 1], p_low[g_low == 2]]
                            Y_dists_high = [p_high[g_high == 0], p_high[g_high == 1], p_high[g_high == 2]]
                            Y_means_low = [np.mean(dist) - np.mean(p_low) for dist in Y_dists_low]
                            Y_means_high = [np.mean(dist) - np.mean(p_high) for dist in Y_dists_high]
                            plt.plot(X_domain, Y_means_low, "-o", markersize=10, label = "below average smoking")
                            plt.plot(X_domain, Y_means_high, "-o", markersize=10, label = "above average smoking")
                            plt.xlabel("additive genotype", fontsize=14)
                            plt.ylabel("average latent phenotype value", fontsize=14)
                            plt.legend()
                            plt.title(rsID + " " + all_E_names[j] + " " + "phenotype" + str(i), fontsize=20)
                            plt.xticks(X_domain, fontsize=12)  # Set x-axis ticks and increase font size
                            plt.yticks(fontsize=12)  # Increase font size for y-axis ticks
                            plt.savefig("figureS4" + letter + ".png")
                            #plt.savefig("top_hits_" + rsID + "_" + all_E_names[j] + "_" + "phenotype" + str(i) + ".png")
                            plt.clf()                      

temp_df = [standard_p_values_rsids, standard_p_values_E_ind, standard_p_values_GxE, standard_p_values_pheno]
temp_df = pd.DataFrame(np.array(temp_df).T)
temp_df.columns = ["rsID", "env_factor", "pval_null", "pheno_index"]
temp_df["pheno_index"] = temp_df["pheno_index"].astype(int)
GxE_info = GxE_effects.merge(temp_df, on = ["rsID", "env_factor", "pheno_index"], how = "inner")
...
figS4_info = GxE_info.loc[GxE_info["pval_null"].astype(float) > 0.001, :]
figS4_info = figS4_info.sort_values(by = "pEDGE2")[["rsID", "pEDGE2", "env_factor", "pheno_index", "chr"]]
figS4_info.to_csv("figS4_info.txt", sep = "\t", header = True, index = False)
# figS4_info.to_numpy()[0:20, 0]

# some commentary
# I wanted to see if miRNA associated SNPs were enriched for heterosis. They are not.
# They are also not obviously enriched for any particular phenotype or enviornmental factor.
# I didn't assess the second claim statistically though.  
p_smoking = GxE_info.loc[GxE_info["env_factor"] == "smoking", ["rsID", "pEDGE2", "pval_null", "pheno_index"]]
mir_rsIDs = pd.read_csv("MIRS_rsids.txt", delimiter = "\t")
mir_rsIDs = np.unique(mir_rsIDs.loc[mir_rsIDs["annot"] == "intronic", "rsID"])
mir_inds =  p_smoking["rsID"].isin(mir_rsIDs)
p_smoking_mirs = p_smoking.loc[mir_inds, :]
non_mir_inds =  p_smoking["rsID"].isin(mir_rsIDs) == False
p_smoking_non_mirs = p_smoking.loc[non_mir_inds, :]
mean_non_mir_pEDGE2 = -np.log10(p_smoking_non_mirs["pEDGE2"].astype(float))
mean_non_mir_pnull = -np.log10(p_smoking_non_mirs["pval_null"].astype(float))
mean_mir_pEDGE2 = -np.log10(p_smoking_mirs["pEDGE2"].astype(float))
mean_mir_pnull = -np.log10(p_smoking_mirs["pval_null"].astype(float))
mwu(mean_non_mir_pnull, mean_mir_pnull, alternative = 'greater')
...
pd.DataFrame(X).to_csv("step10e_PCA_PRS.txt", sep = "\t", header = False, index = False)
...
prefix = "../step10_get_significant_SNPs_NN/"
X2_NN = pd.read_csv(prefix + "step10e_NN_PRS.txt", delimiter = "\t", header = None).to_numpy()
X3 = np.concatenate([X2, X2_NN[keepers, :]], axis = 1)
