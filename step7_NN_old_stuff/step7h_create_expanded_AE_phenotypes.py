...
class simpleAE(torch.nn.Module):
    ...
    def __init__(self, input_shape, output_shape, latent_dims, dop):
        ...
        dims_in = [input_shape] + latent_dims
        dims_out = np.flip(dims_in)
        dims_out[-1] = output_shape
...
name = "CV_expandedAE_" + str(dop) + "dop_" + str(rs) + "rs_"  + str(ws) + "ws_" + str(dim1) + "d1_" + str(dim2) + "d2.txt"
model_path = "expandedAE_CV_fold_networks/" + name[:-4] + ".model"
folds = [1, 2, 3, 4, 5]
for train_inds, test_inds, CV in zip(training_inds, testing_inds, folds):
    ...
    network = simpleAE(in_dim, out_dim, [dim1, dim2, 15], dop) 
    ...
    model_path = "expandedAE_CV_fold_networks/fold" + str(CV) + "_" + name[:-4] + ".model"
    if os.path.exists(model_path):
        network.load_state_dict(torch.load(model_path))
    
    for k in range(500):
        time_sum = 0
        batched_data, num_batches = get_batches(torch.tensor(np.concatenate([X_train, Y_train], axis = 1)).float(), 
                                                torch.tensor(np.concatenate([X_test, Y_test], axis = 1)).float(), 
    ...
    network.eval()
    X1 = torch.tensor(np.concatenate([X_train, Y_train], axis = 1)[:int(len(Y_test)/1)]).float()
    ...
    X2 = torch.tensor(np.concatenate([X_test, Y_test], axis = 1)[:int(len(Y_test)/1)]).float()
    ...
    fold_errs.to_csv("expandedAE_fold_error_dfs/fold" + str(CV) + "_" + name, sep = "\t", header = True, index = False)
