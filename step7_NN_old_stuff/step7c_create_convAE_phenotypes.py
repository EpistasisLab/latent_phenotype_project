...
name = "CV_" + str(rs) + "rs_"  + str(ws) + "ws_" + str(dim1) + "d1_" + str(dim2) + "d2.txt"
model_path = "CV_fold_networks/" + name[:-4] + ".model"
folds = [1, 2, 3, 4, 5]
for train_inds, test_inds, CV in zip(training_inds, testing_inds, folds):
    ...
    network = simpleAE(50, out_dim, [dim1, dim2, 15]) 
    ...
    model_path = "CV_fold_networks/fold" + str(CV) + "_" + name[:-4] + ".model"
    ...
    fold_errs.to_csv("fold_error_dfs/fold" + str(CV) + "_" + name, sep = "\t", header = True, index = False)
...
plt.legend()
image_name = "aaa_" + str(rs) + "rs_"  + str(ws) + "ws_" + str(dim1) + "d1_" + str(dim2) + "d2.png"
